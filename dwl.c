...
typedef struct {
  bool in_use;
  struct wlr_seat *seat;
  struct wl_resource *virtual_touch;
  struct wlr_surface *touch_surface;
...
static void focusclient(Client *c, int lift, struct wlr_seat *seat);
static void focusmon(const Arg *arg);
static void focusstack(const Arg *arg);
static Client *focustop(Monitor *m);
static void fullscreennotify(struct wl_listener *listener, void *data);
static uint32_t generate_serial(void);
...
static void motionnotify(uint32_t time, struct wlr_seat *seat);
...
static void pointerfocus(Client *c, struct wlr_surface *surface, double sx,
                         double sy, uint32_t time, struct wlr_seat *seat);
...
static int seat_index = MAX_NUM_SESSIONS;
static Seat seats[MAX_NUM_SESSIONS + 1];
...
/* should be at least the same number of xwayland instances as users to work
 * around X11 multiseat limitation */
static XWayland xwaylands[MAX_NUM_SESSIONS];
...
static void
force_scene_buffer_send_frame_done(struct wlr_scene_buffer *scene_buffer,
                                   struct timespec *now) {
...
static void force_scene_node_send_frame_done(struct wlr_scene_node *node,
  ...
  if (node->type == WLR_SCENE_NODE_BUFFER) {
    ...
    wl_list_for_each(child, &scene_tree->children, link) {
      force_scene_node_send_frame_done(child, now);
    }
  }
}
...
void get_window_info(struct wl_client *client,
  ...
  struct wl_resource *resource =
      wl_resource_create(client, &zcompositor_v1_interface, version, id);
  ...
  wl_list_for_each(c, &fstack, flink) {
    ...
    zcompositor_v1_send_window_info(resource, c->serial, client_get_pid(c),
                                    title);
  }
...
void set_window_area(struct wl_client *client,
                     struct wl_resource *manager_resource, uint32_t window_id,
  ...
  wlr_log(WLR_INFO, "Got set window area request for %u: [%d %d %d %d]",
          window_id, x, y, width, height);

  wl_list_for_each(c, &fstack, flink) {
    if (c->serial == window_id) {
      wlr_log(WLR_INFO, "Resizing window %u", window_id);
      if (geom.x != c->target_area.x || geom.y != c->target_area.y ||
...
void raise_window(struct wl_client *client,
                  struct wl_resource *manager_resource, uint32_t window_id) {
  Client *c;
  struct wlr_box geom;
  ...
  wl_list_for_each(c, &fstack, flink) {
    ...
    if (c->serial == window_id) {
      if (!client_is_x11(c)) {
        ...
        wlr_log(WLR_INFO, "Configure %u to %dx%d or %dx%d", window_id,
                c->target_area.width, c->target_area.height, c->geom.width,
                c->geom.height);
        wlr_xdg_toplevel_set_size(c->surface.xdg->toplevel, geom.width,
                                  geom.height);
      }
      if (render_scene) {
...
static void compositor_manager_bind(struct wl_client *client, void *data,
                                    uint32_t version, uint32_t id) {
  struct compositor_manager_v1 *manager = data;

  struct wl_resource *resource = wl_resource_create(
      client, &zcompositor_manager_v1_interface, version, id);
  if (resource == NULL) {
...
struct compositor_manager_v1 *
compositor_manager_v1_create(struct wl_display *display) {
  struct compositor_manager_v1 *manager =
      ecalloc(1, sizeof(struct compositor_manager_v1));
  wl_signal_init(&manager->events.destroy);
  wlr_log(WLR_DEBUG, "Creating comp man v1 global");
  manager->global = wl_global_create(display, &zcompositor_manager_v1_interface,
                                     1, manager, compositor_manager_bind);
...
static void window_capture_manager_bind(struct wl_client *client, void *data,
                                        uint32_t version, uint32_t id) {
  struct window_capture_manager_v1 *manager = data;

  struct wl_resource *resource = wl_resource_create(
      client, &zwindow_capture_manager_v1_interface, version, id);
  if (resource == NULL) {
    wl_client_post_no_memory(client);
    return;
  }
  wl_resource_set_implementation(resource, &window_capture_manager_impl,
                                 manager, NULL);
}

struct window_capture_manager_v1 *
window_capture_manager_v1_create(struct wl_display *display) {
  struct window_capture_manager_v1 *manager =
      ecalloc(1, sizeof(struct window_capture_manager_v1));
  wl_signal_init(&manager->events.destroy);
  wlr_log(WLR_DEBUG, "Creating window capture man v1 global");
  manager->global =
      wl_global_create(display, &zwindow_capture_manager_v1_interface, 1,
                       manager, window_capture_manager_bind);
  if (manager->global == NULL) {
    wlr_log(WLR_ERROR, "Global is null");
    free(manager);
  }
...
static struct wlr_dmabuf_buffer *
dmabuf_buffer_from_buffer(struct wlr_buffer *wlr_buffer) {
...
static struct wlr_dmabuf_buffer *
dmabuf_buffer_create(struct wlr_dmabuf_attributes *dmabuf) {
  struct wlr_dmabuf_buffer *buffer = calloc(1, sizeof(*buffer));
  if (buffer == NULL) {
    return NULL;
  }
  wlr_buffer_init(&buffer->base, &dmabuf_buffer_impl, dmabuf->width,
                  dmabuf->height);
...
static void frame_client_handle_commit(struct wl_listener *listener,
  ...
  if (capture_buffer) {
    pass =
        wlr_renderer_begin_buffer_pass(surface->renderer, capture_buffer, NULL);
    if (pass == NULL) {
      wlr_log(WLR_ERROR,
    ...
    wlr_log(WLR_ERROR, "No source (%p) or destination buffer (%p)", source,
            capture_buffer);
  }
...
uint32_t align64(uint32_t value) { return (((value + 63) >> 6) << 6); }
...
void copy_window_frame(struct wl_client *client,
  ...
  assert(wl_resource_instance_of(manager_resource,
                                 &zwindow_capture_manager_v1_interface,
                                 &window_capture_manager_impl));
  ...
  frame->resource = wl_resource_create(
      client, &zwindow_dmabuf_frame_v1_interface, version, id);
  ...
  wl_list_for_each(c, &fstack, flink) {
    if (c->serial == window_id) {
      wl_signal_add(&client_surface(c)->events.client_commit,
                    &frame->client_commit);
  ...
  if (frame->client == NULL) {
    zwindow_dmabuf_frame_v1_send_cancel(
        frame->resource, ZWINDOW_DMABUF_FRAME_V1_CANCEL_REASON_PERMANENT);
    frame_destroy(frame);
    close(fd);
  } else {
    if (!wait_for_next_frame) {
      frame_client_handle_commit(&frame->client_commit,
                                 client_surface(frame->client));
    }
    /* Let client know a frame has been rendered */
    clock_gettime(CLOCK_MONOTONIC, &now);
    force_scene_node_send_frame_done(&frame->client->scene_surface->node, &now);
  }
}

static const struct zvirtual_touch_v1_interface virtual_touch_impl;

static void virtual_touch_down(struct wl_client *client,
                               struct wl_resource *resource, uint32_t serial,
                               uint32_t time, uint32_t window_id, int32_t id,
                               int32_t x, int32_t y) {
  Seat *s = wl_resource_get_user_data(resource);
  Client *c = NULL;
  wl_list_for_each(c, &fstack, flink) {
    wlr_log(WLR_INFO, "Checking window %u", c->serial);
    if (c->serial == window_id) {
      wlr_log(WLR_INFO, "Found matching window %u", c->serial);
      s->touch_surface = client_surface(c);
      break;
    }
  }
  ...
  wlr_log(WLR_INFO, "Touch down [%d] (%d, %d) at %p", id, x, y,
          s->touch_surface);
  if (!c) {
    wlr_log(WLR_ERROR, "Touch down: could not find surface for window id %u",
            window_id);
    return;
  }

  wlr_idle_notifier_v1_notify_activity(idle_notifier, s->seat);

  if (c && (!client_is_unmanaged(c) || client_wants_focus(c)))
    focusclient(c, 1, s->seat);

  if (!s->touch_surface) {
    return;
  }

  wlr_seat_touch_notify_down(s->seat, s->touch_surface, time, id, x, y);
}

static void virtual_touch_up(struct wl_client *client,
                             struct wl_resource *resource, uint32_t serial,
                             uint32_t time, int32_t id) {
  Seat *s = wl_resource_get_user_data(resource);
  wlr_log(WLR_INFO, "Touch up [%d] at %p", id, s->touch_surface);

  wlr_idle_notifier_v1_notify_activity(idle_notifier, s->seat);

  s->touch_surface = NULL;

  wlr_seat_touch_notify_up(s->seat, time, id);
}

static void virtual_touch_motion(struct wl_client *client,
                                 struct wl_resource *resource, uint32_t time,
                                 int32_t id, int32_t x, int32_t y) {
  Seat *s = wl_resource_get_user_data(resource);
  wlr_log(WLR_INFO, "Touch move [%d] (%d, %d) at %p", id, x, y,
          s->touch_surface);

  wlr_seat_touch_notify_motion(s->seat, time, id, x, y);
}

static void virtual_touch_destroy_resource(struct wl_resource *resource) {
  // TODO: Implement if needed
}

static const struct zvirtual_touch_v1_interface virtual_touch_impl = {
    .down = virtual_touch_down,
    .up = virtual_touch_up,
    .motion = virtual_touch_motion};
...
static void handle_register_input_device(struct wl_client *client,
  ...
  wlr_log(WLR_INFO, "Registering input device %s at seat index %d", name,
          index);
}
...
static void handle_request_virtual_touch(struct wl_client *client,
  ...
  wl_resource_set_implementation(touch_resource, &virtual_touch_impl,
                                 &seats[index], virtual_touch_destroy_resource);
  wlr_log(WLR_INFO, "New virtual touch [%d]", index);
  // TODO: We might leak memory here. Does the resource get destroyed when
  // client disconnects?
  seats[index].virtual_touch = touch_resource;
}
...
static const struct zinput_device_seat_mapper_v1_interface seat_mapper_impl = {
    ...
    .request_virtual_touch = handle_request_virtual_touch,
    .destroy = seat_mapper_handle_destroy,
};

static void seat_mapper_bind(struct wl_client *client, void *data,
                             uint32_t version, uint32_t id) {
  struct input_device_seat_mapper_v1 *sm = data;

  struct wl_resource *resource = wl_resource_create(
      client, &zinput_device_seat_mapper_v1_interface, version, id);
  if (resource == NULL) {
    wl_client_post_no_memory(client);
    return;
  }
  wl_resource_set_implementation(resource, &seat_mapper_impl, sm, NULL);
}

static void seat_mapper_handle_display_destroy(struct wl_listener *listener,
                                               void *data) {
...
struct input_device_seat_mapper_v1 *
input_device_seat_mapper_v1_create(struct wl_display *display) {
  struct input_device_seat_mapper_v1 *sm =
      ecalloc(1, sizeof(struct input_device_seat_mapper_v1));
  wl_list_init(&sm->registered_seats);
  wl_signal_init(&sm->events.destroy);
  wlr_log(WLR_DEBUG, "Creating seat mapper v1 global");
  sm->global =
      wl_global_create(display, &zinput_device_seat_mapper_v1_interface, 1, sm,
                       seat_mapper_bind);
  if (sm->global == NULL) {
    wlr_log(WLR_ERROR, "Global is null");
    free(sm);
...
void seatinit(struct wl_display *display, int i) {
  ...
  LISTEN(&cursor->events.motion_absolute, &s->events.cursor_motion_absolute,
         motionabsolute);
  ...
  s->events.request_set_psel.notify = setpsel;
  s->events.request_set_sel.notify = setsel;
  s->events.request_cursor.notify = setcursor;
  s->events.request_start_drag.notify = requeststartdrag;

  LISTEN(&seat->events.request_set_cursor, &s->events.request_cursor,
         setcursor);
  LISTEN(&seat->events.request_set_selection, &s->events.request_set_sel,
         setsel);
  LISTEN(&seat->events.request_set_primary_selection,
         &s->events.request_set_psel, setpsel);
  LISTEN(&seat->events.request_start_drag, &s->events.request_start_drag,
         requeststartdrag);
  wl_signal_add(&seat->events.start_drag, &start_drag);
}

Seat *currentseat(void) { return &seats[seat_index]; }
  ...
  for (i = 0; i < LENGTH(layers_above_shell); i++) {
    wl_list_for_each_reverse(layersurface, &m->layers[layers_above_shell[i]],
      ...
      if (!locked &&
        ...
        focusclient(NULL, 0, seat);
        exclusive_focus = layersurface;
        client_notify_enter(layersurface->layer_surface->surface, seat);
        return;
      }
    }
  }
...
void axisnotify(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.cursor_axis);
...
void buttonpress(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.cursor_button);
  struct wlr_seat *seat = s->seat;
...
void checkidleinhibitor(struct wlr_surface *exclude) {
  ...
  // {
  // 	struct wlr_surface *surface =
  // wlr_surface_get_root_surface(inhibitor->surface); 	struct wlr_scene_tree
  // *tree = surface->data; 	if (exclude != surface &&
...
void commitnotify(struct wl_listener *listener, void *data) {
  ...
  // attribs.height, attribs.fd[0]); 	struct wlr_texture *source =
  // wlr_gles2_texture_from_dmabuf(s->renderer, &attribs);
...
void createkeyboard(struct wlr_keyboard *keyboard) {
  ...
  wlr_log(WLR_INFO, "Added keyboard %p to seat %p", (uint64_t)kb,
...
void createlocksurface(struct wl_listener *listener, void *data) {
  ...
  if (m == selmon)
    client_notify_enter(lock_surface->surface, seat);
}
...
void createmon(struct wl_listener *listener, void *data) {
  ...
  if (!wlr_output_commit_state(wlr_output, &pending)) {
    ...
    wlr_log(WLR_ERROR, "Failed to set output mode [%d %d %f]", mode_width,
            mode_height, mode_refresh);
  }
...
void createtouch(struct wlr_touch *touch) {
  ...
  if (wlr_input_device_is_libinput(&touch->base)) {
    struct libinput_device *libinput_device =
        (struct libinput_device *)wlr_libinput_get_device_handle(&touch->base);
...
void cursorframe(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.cursor_frame);
  struct wlr_seat *seat = s->seat;
...
void destroydragicon(struct wl_listener *listener, void *data) {
  ...
  focusclient(focustop(selmon), 1, seat);
  motionnotify(0, seat);
}
...
void destroylock(SessionLock *lock, int unlock) {
  ...
  focusclient(focustop(selmon), 0, seat);
  motionnotify(0, seat);
...
void destroylocksurface(struct wl_listener *listener, void *data) {
  ...
  if (locked && cur_lock && !wl_list_empty(&cur_lock->surfaces)) {
    ...
    client_notify_enter(surface->surface, seat);
  } else if (!locked) {
    focusclient(focustop(selmon), 1, seat);
  } else {
...
void focusclient(Client *c, int lift, struct wlr_seat *seat) {
  ...
  wlr_log(WLR_DEBUG, "Focusclient, surface: %p, seat: %p, x11: %u",
...
uint32_t generate_serial(void) {
  struct timespec now;
  clock_gettime(CLOCK_MONOTONIC, &now);
  return ((now.tv_sec & 0xffff) << 16) | (now.tv_nsec & 0xffff);
}
...
void handlesig(int signo) {
  if (signo == SIGCHLD) {
    ...
    while (!waitid(P_ALL, 0, &in, WEXITED | WNOHANG | WNOWAIT) && in.si_pid) {
      bool waiting = true;
      for (int i = 0; i < MAX_NUM_SESSIONS; ++i)
        waiting = waiting && (!xwaylands[i].xwayland ||
                              in.si_pid != xwaylands[i].xwayland->server->pid);
      if (!waiting)
        break;
      waitpid(in.si_pid, NULL, 0);
    }
...
void inputdevice(struct wl_listener *listener, void *data) {
  ...
  if (seat_index != MAX_NUM_SESSIONS) {
    wl_list_remove(&rs->link);
    free(rs);
  } else {
    wlr_log(WLR_ERROR, "New input device %s not registered - skipping",
            device->name);
    return;
  }

  s = &seats[seat_index];
  wlr_log(WLR_INFO, "New input device %s at seat index %d of type %u, seat %p",
          device->name, seat_index, device->type, s);
...
void motionabsolute(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.cursor_motion_absolute);
  struct wlr_seat *seat = s->seat;
  struct wlr_cursor *cursor = s->cursor;
  ...
  motionnotify(event->time_msec, seat);
}

void motionnotify(uint32_t time, struct wlr_seat *seat) {
  ...
  // Can probably remove this since we always expect there to be a window
  // active anyway if (!surface && !seat->drag && (!cursor_image ||
  // strcmp(cursor_image, "left_ptr")))
  // 	wlr_xcursor_manager_set_cursor_image(cursor_mgr, (cursor_image =
  // "left_ptr"), cursor);

  pointerfocus(c, surface, sx, sy, time, seat);
}

void motionrelative(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.cursor_motion);
  struct wlr_seat *seat = s->seat;
  struct wlr_cursor *cursor = s->cursor;
...
void requeststartdrag(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.request_start_drag);
  struct wlr_seat *seat = s->seat;
...
void setcursor(struct wl_listener *listener, void *data) {
  ...
  if (event->seat_client == seat->pointer_state.focused_client)
    wlr_cursor_set_surface(cursor, NULL, event->hotspot_x, event->hotspot_y);
}
...
void setmode(struct wlr_output *output, struct wlr_output_state *pending,
             int width, int height, float refresh_rate, bool custom) {
...
void setpsel(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.request_set_psel);
  struct wlr_seat *seat = s->seat;
...
void setsel(struct wl_listener *listener, void *data) {
  Seat *s = wl_container_of(listener, s, events.request_set_sel);
  struct wlr_seat *seat = s->seat;
...
static bool global_filter(const struct wl_client *client,
                          const struct wl_global *global, void *user_data) {
...
void setup(void) {
  ...
  compositor =
      wlr_compositor_create(dpy, 5, drw); // Some clients have issues with 6
  ...
  for (i = 0; i < MAX_NUM_SESSIONS; ++i) {
    ...
    wlr_log(WLR_INFO, "Created xwayland %s",
    ...
    if (xwaylands[i].xwayland) {
      ...
      wl_signal_add(&xwaylands[i].xwayland->events.ready,
                    &xwaylands[i].events.ready);
      wl_signal_add(&xwaylands[i].xwayland->events.new_surface,
                    &xwaylands[i].events.new_surface);
...
void touchdown(struct wl_listener *listener, void *data) {
  ...
  // xytonode(event->x * mode_width, event->y * mode_height, &surface, &c,
  // NULL, &sx, &sy);

  // if (c && (!client_is_unmanaged(c) || client_wants_focus(c)))
  // 	focusclient(c, 1, seat);

  // wlr_log(WLR_DEBUG, "Touchdown [%d] (%lf, %lf) => (%lf, %lf), surface: %p,
  // seat: %p", event->touch_id, event->x, event->y, sx, sy, surface, seat);
...
void touchmotion(struct wl_listener *listener, void *data) {
  ...
  // wlr_log(WLR_DEBUG, "Touchmotion [%d] (%lf, %lf) => (%lf, %lf), seat: %p",
...
void touchup(struct wl_listener *listener, void *data) {
  ...
  // wlr_log(WLR_DEBUG, "Touchup [%d], target seat: %p, wlrseat: %p, cursor:
  // %p, found seat: %p, found wlrseat: %p, found cursor: %p", event->touch_id,
  // currentseat(), currentseat()->seat, currentseat()->cursor, s, s->seat,
  // s->cursor);
...
void updatemons(struct wl_listener *listener, void *data) {
  ...
  if (selmon && selmon->wlr_output->enabled) {
    wl_list_for_each(c, &clients,
                     link) if (!c->mon && client_surface(c)->mapped)
        setmon(c, selmon, c->tags);
    focusclient(focustop(selmon), 1, seat);
    if (selmon->lock_surface) {
      client_notify_enter(selmon->lock_surface->surface, seat);
...
void xwaylandready(struct wl_listener *listener, void *data) {
  ...
  if ((xcursor = wlr_xcursor_manager_get_xcursor(cursor_mgr, "default", 1)))
    wlr_xwayland_set_cursor(
        xwayland->xwayland, xcursor->images[0]->buffer,
